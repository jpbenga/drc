...
function escapeHtml(str) {
  ...
  // Shrinkage bayésien entraînable (C = confidence_shrinkage)
  ...
  // Dixon–Coles tau aligned with optimizer_high_volume.py
  ...
  function detectImpactAbsences(match, meta, side) {
    ...
    injuries.forEach((inj) => {
      ...
      const isTopDef = inList(topDefenders, pid, pName) || inList(topGoalkeepers, pid, pName);
      if (isTopDef) {
        tags.push("Defensive Anchor");
        def += 1;
        defensiveImpact += 1;
      }
  
      const player = playerRatings?.find((p) => normId(p.id) === normId(pid));
      ...
      if (player && (position === "Defender" || position === "Goalkeeper")) {
        if (rating != null && !Number.isNaN(rating) && rating >= 7.0) {
          if (!tags.includes("Key Defender/GK")) tags.push("Key Defender/GK");
          def += 1;
          defensiveImpact += 1;
        }
      }
      ...
      absences.push({
        id: pid,
        name: findName(pid, pName || null),
        position,
        rating,
        type: inj.type || "—",
        reason: inj.reason || inj.detail || inj.comment || "—",
        tags,
        impact_off: off,
        impact_def: def,
      });
  ...
  // ============================================================================
  // WALK-FORWARD CALIBRATION (Platt scaling on logit(p_raw))
  // ============================================================================
  class PlattCalibrator {
    constructor(opts = {}) {
      this.a = 1.0; // slope
      this.b = 0.0; // intercept
      this.lr = opts.lr ?? 0.02; // learning rate
      this.reg = opts.reg ?? 0.001; // L2 toward (a=1,b=0)
      this.clipA = opts.clipA ?? [0.25, 3.0];
      this.clipB = opts.clipB ?? [-2.0, 2.0];
      this.n = 0;
    }
    predict(pRaw) {
      const z = logit(pRaw);
      const q = sigmoid(this.a * z + this.b);
      return clamp(q, 1e-6, 1 - 1e-6);
    }
    update(pRaw, y) {
      const z = logit(pRaw);
      const q = sigmoid(this.a * z + this.b);
      const err = q - (y ? 1 : 0);
      const da = err * z + this.reg * (this.a - 1.0);
      const db = err + this.reg * (this.b - 0.0);
  ...
  // ============================================================================
  // SCORE MATRIX (0..MAX) with DC + clamp + renormalization
  // ============================================================================
  function buildScoreMatrix(lh, la, rho, MAX) {
    const probs = [];
    let sumRaw = 0;
    let minRaw = Infinity;
    ...
    return { probs, sumRaw, minRaw };
  }
  
  function idx(i, j, MAX) {
    return i * (MAX + 1) + j;
  }
  
  // ============================================================================
  // POISSON PRO + RAW SUBMARKETS (0..1) + Impact debug
  // ============================================================================
  function calculatePoissonPro(hID, aID, hElo, aElo, tracker, match, metaHome, metaAway) {
    ...
    const MAX = Number(PARAMS.max_goals ?? 20);
    const { probs, sumRaw, minRaw } = buildScoreMatrix(lh, la, PARAMS.rho, MAX);
    ...
    for (let i = 0; i <= MAX; i++) {
      for (let j = 0; j <= MAX; j++) {
        ...
        if (i > 0 && j > 0) pBTTS += p;
        if ((i + j) <= 2) pUnder25 += p;
        if (i > 0) pHomeScores += p;
        if (j > 0) pAwayScores += p;
  
        scoreProbs.push({ score: `${i}-${j}`, prob: p });
      }
    }
  
    // Home goals <=1: sum i=0,1 across j
    for (let j = 0; j <= MAX; j++) {
      pHomeLE1 += probs[idx(0, j, MAX)] + probs[idx(1, j, MAX)];
      pAwayLE1 += probs[idx(j, 0, MAX)] + probs[idx(j, 1, MAX)];
    }
  ...
  function runBacktest() {
    ...
    for (const item of all) {
      ...
      if (tracker[hID].xg.length >= PARAMS.min_matches && tracker[aID].xg.length >= PARAMS.min_matches) {
        ...
        const res = calculatePoissonPro(hID, aID, hElo, aElo, tracker, m, metaByLeague[lid], metaByLeague[lid]);
        if (res) {
