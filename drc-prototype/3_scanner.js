...
async function startScanner() {
    console.clear();
    console.log(`\n🚀 SCANNER PRO - DESIGN 'CASH' ACTIVÉ`);
    ...
    // 2. Scan (J et J+1)
    console.log(`\n🔄 ANALYSE DES MATCHS (JOUR J et J+1)...`);
    const newBets = await runScan(history);
    
    // 3. Sauvegarde
    saveBets(newBets);

    // 4. Serveur Visuel (Design "Cash.js")
    const server = http.createServer((req, res) => {
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(generateHTML(newBets));
    });

    server.listen(PORT, () => {
        console.log(`\n🌐 RAPPORT VISUEL : http://localhost:${PORT}`);
...
async function runScan(history) {
    let detectedBets = [];
            ...
            try {
                ...
                for (const m of mRes.data.response) {
                    ...
                    if (sdmData && Math.abs(sdmData.val) >= 5) { 
                        ...
                        const minOddReq = USER_STRATEGY.MIN_ODDS[tier];
                        const stakeToPlace = USER_STRATEGY.STAKES[tier];
...
function saveBets(newBets) {
    ...
    console.log(`\n💾 SAUVEGARDE : ${addedCount} nouveaux paris ajoutés dans ${PENDING_FILE}`);
}
...
// =================================================================
// 🎨 DESIGN "CASH.JS" (ADAPTÉ POUR LE SCANNER)
// =================================================================
function generateHTML(bets) {
    const cGreen = '#4ade80'; const cMuted = '#94a3b8';
    ...
    <head>
        <meta charset="UTF-8">
        <title>Scanner Pro - Résultats</title>
        ...
        <style>
            ...
            .header h1 { margin: 0; color: #facc15; font-size: 2em; text-transform:uppercase; }
            ...
            /* Badges identiques à Cash.js */
            .badge { padding: 3px 6px; border-radius: 4px; font-size: 0.75em; font-weight: bold; border: 1px solid currentColor; }
            .b-15 { color: #facc15; background: rgba(250,204,21,0.1); } /* SUPREME */
            .b-10 { color: #cbd5e1; background: rgba(203,213,225,0.1); } /* SOLID */
            .b-5  { color: #fb923c; background: rgba(251,146,60,0.1); }  /* VALUE */
    ...
    <div class="header">
        <div>
            ...
            <div style="color:${cMuted}">Scan Live • Jours J & J+1</div>
        </div>
    ...
    <div class="kpi-row">
        <div class="kpi-card">
            ...
            <div class="kpi-val" style="color:#facc15">${bets.length}</div>
        </div>
        <div class="kpi-card">
            <div class="kpi-title">Mise Totale Engagée</div>
    ...
    <table>
        ...
        <tbody>
            ${(() => {
                let rows = '';
                let lastDay = '';
                if(bets.length === 0) return '<tr><td colspan="5" style="text-align:center; padding:30px">Aucune opportunité détectée pour le moment.</td></tr>';
